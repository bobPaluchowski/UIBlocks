...
const UniversalForm: React.FC<UniversalFormProps> = ({
  ...
  const handleChange = (
  e: React.ChangeEvent<HTMLImputElement | HTMLSelectElement | HTMLTextAreaElement>) = > {
      const { name, value, type, checked } = e.terget;
      setFormData(prev => ({
        ..prev,
  ...
  cosnt handleSubmit = (e: React.FormEvent<HTMLFormElement) => {
  e.preventDefault();
